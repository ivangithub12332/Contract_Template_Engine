# Шаблонизатор договоров: frontend-only

Чистая frontend-ветка проекта. В этой ветке лежит только React-приложение без backend-кода.

Полная интеграционная ветка с backend, frontend и общей инструкцией:

```text
https://github.com/ivangithub12332/Contract_Template_Engine/tree/full-project
```

Frontend работает с Laravel API, который должен быть запущен отдельно на `http://localhost:8000/api`.
...
Backend находится не в этой ветке, а в ветке `full-project`.

Чтобы запустить backend, используйте интеграционную ветку:
